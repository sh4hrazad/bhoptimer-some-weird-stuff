 ...
 * shavit-ghost.sp
 *
 * Fixed memory leak, invalid handle error, etc.
 */
...
public Action getUsers(int client, int args)
{
	...
	PrintToChat(client, "[Ghost] users logged in Console");
		...
		case MenuAction_Select:
		{
			...
			if(menu.GetItem(param2, info, sizeof(info)))
			{
				if(StrEqual(info, "Enable/Disable"))
				{
					...
					PrintToChat(client, "[Ghost] Ghost: %s", (ghost[client] ? "Enabled" : "Disabled"));
				}
				...
				else if(StrEqual(info, "Mode"))
				{
					...
					PrintToChat(client, "[Ghost] Ghost Mode: %s", (ghostMode[client] == 0 ? "Personal Best" : "World Record"));
					
					if(ghostMode[client] == 0)
						PrintToChat(client, "[Ghost] Complete the map and a Ghost will appear.");
				}
	...
	{		
		case MenuAction_Select:
		{
			...
			if(menu.GetItem(param2, info, sizeof(info)))
			{
				...
				{
					beamColor[client] =  { 255, 0, 0, 255 };
					Format(text, sizeof(text), "Red");
				}
				else if(StrEqual(info, "Green"))
				{
					beamColor[client] =  { 0, 255, 0, 255 };
					Format(text, sizeof(text), "Green");
				}
				else if(StrEqual(info, "Blue"))
				{
					beamColor[client] =  { 0, 0, 255, 255 };
					Format(text, sizeof(text), "Blue");
				}
				...
				PrintToChat(client, "[Ghost] Trail Color: %s", text);
...
public void Shavit_OnStyleChanged(int client, int oldstyle, int newstyle, int track, bool manual)
{
	...
	{				
		...
		if(ar)
		{
			int iPreFrames = Shavit_GetReplayPreFrames(newstyle, track);

			for (int i = 0; i < iPreFrames; i++)
			{
				ar.Erase(0);
			}
...
public void Shavit_OnEnterZone(int client, int type, int track, int id, int entity, int data)
{
	if(type == Zone_Start && ghost[client])
	{
		...
		{
			int playerStyle = Shavit_GetBhopStyle(client);
			int playerTrack = Shavit_GetClientTrack(client);

			ArrayList ar = Shavit_GetReplayFrames(playerStyle, playerTrack);
		
			if(ar)
			{
				int iPreFrames = Shavit_GetReplayPreFrames(playerStyle, playerTrack);
